...
export const SavePresetModal: React.FC<{
    ...
    const handleConfirm = () => {
        ...
        const preset: AdvancedPreset = {
            ...
            includeStyle,
            includeContent,
            coverState: initialData?.coverState || (includeStyle ? {
            ...
            <div className="bg-white w-full h-[calc(100dvh-3.5rem)] sm:h-[85vh] sm:w-[500px] rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in-95 transition-all">
                <div className="flex flex-col h-full w-full">
                    <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
                        <h3 className="font-bold text-gray-800">{initialData ? '编辑排版预设' : '新建排版预设'}</h3>
                    ...
                    <div className="p-5 overflow-y-auto custom-scrollbar space-y-5 flex-1 min-h-0">
                        <div>
                            ...
                            <input value={name} onChange={e => setName(e.target.value)} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:border-purple-300 transition-all" placeholder="如：判词专用排版" autoFocus />
                        </div>
                        ...
                        <div className="border-t border-gray-100 pt-4">
                            <div className="flex justify-between items-center mb-3">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] text-gray-400">设置关键词触发自动布局</span>
                                </div>
                                <button onClick={scanTextForRules} className="flex items-center gap-1 text-[10px] bg-purple-50 text-purple-600 px-2 py-1 rounded-md border border-purple-100 hover:bg-purple-100 transition-colors">
                                    <EyeDropperIcon className="w-3 h-3" /> {isScanning ? '扫描中...' : '扫描当前排版'}
                                </button>
                            </div>
                            <div className="space-y-3 pb-2">
                                ...
                                <button onClick={() => setRules(prev => [...prev, { id: `rule_${Date.now()}`, name: '新规则', pattern: '', formatting: {}, scope: 'match', isActive: true }])} className="w-full py-2 border border-gray-200 rounded-lg text-xs text-gray-500 hover:bg-gray-50 flex items-center justify-center gap-1 transition-colors">
                    ...
                    <div className="p-4 border-t border-gray-100 bg-gray-50/30 shrink-0">
                        <button onClick={handleConfirm} disabled={!name.trim()} className="w-full py-3 bg-gray-900 text-white rounded-xl font-bold text-sm active:scale-95 disabled:opacity-50 transition-all shadow-lg">
                            {initialData ? '保存修改' : '保存排版预设'}
                        </button>
                    </div>
                </div>
            </div>
...
export const MobilePresetPanel: React.FC<PresetPanelProps> = ({ presets, onSavePreset, onDeletePreset, onApplyPreset, state, onFormatText }) => {
    ...
    const handleEdit = (preset: AdvancedPreset) => {
            ...
            <div className="px-4 py-2 flex justify-between items-center border-b border-gray-100 shrink-0 h-10">
                ...
                {activeRules.length > 0 && (
                     <button onClick={() => onFormatText(activeRules)} className="flex items-center gap-1 bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-3 py-1 rounded-full text-[10px] font-bold shadow-sm active:scale-95 transition-all">
                        <SparklesIcon className="w-3 h-3" /> 应用排版规则
                    </button>
                )}
            </div>
            <div className="flex-1 overflow-x-auto overflow-y-hidden flex items-center gap-3 px-4 py-2 custom-scrollbar">
                <div onClick={handleCreateNew} className="shrink-0 w-32 h-24 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center gap-2 text-gray-400 cursor-pointer hover:border-purple-500 hover:text-purple-500 hover:bg-purple-50 transition-all active:scale-95 bg-gray-50/50">
                    <PlusIcon className="w-6 h-6" /> <span className="text-[10px] font-bold">新建预设</span>
                </div>
                {presets.map((preset) => {
                    const isActive = activePresetId === preset.id;
                    return (
                        <div key={preset.id} className={`relative shrink-0 w-32 h-24 bg-white border rounded-lg shadow-sm flex flex-col overflow-hidden transition-all duration-200 ${isActive ? 'border-purple-500 ring-1 ring-purple-500' : 'border-gray-200'}`} onClick={() => handleApply(preset)}>
                            <div className="p-2.5 flex flex-col h-full">
                                ...
                                <div className="flex gap-1 flex-wrap mb-auto">
                                    ...
                                    {preset.rules.length > 0 && <span className="text-[8px] bg-purple-50 text-purple-600 px-1 rounded border border-purple-100">规则x{preset.rules.length}</span>}
                                </div>
                                <div className="mt-auto flex justify-between items-center pt-1 border-t border-gray-50">
                                     <button onClick={(e) => { e.stopPropagation(); handleEdit(preset); }} className="text-gray-400 hover:text-purple-500 p-1">
                                        <PencilSquareIcon className="w-3.5 h-3.5" />
                                     </button>
                                     <button onClick={(e) => { e.stopPropagation(); onDeletePreset(preset.id); }} className="text-gray-300 hover:text-red-500 p-1">
